...
// kan door coaches gedaan worden
...
if(isset($_POST['nieuwdossier-submit']))
{
	...
	else
	{
		...
		mysqli_stmt_bind_param($stmt, 'ssii', $dossertitel, $dossieromschrijving, $userid, $SESSION['clientid']);// s staat voor string i staat voor integer
...
//SEBAS ONDERSTAANDE CODE KOPIEREN !! Is de SELECT functie voor weergeven dossiers horende bij client
$SESSION['clientid'];
//query
	$viewdossier = "SELECT Filename FROM bestand 
					WHERE User_Client = ?;";
					/* bovenstaande kan ook zijn
					"SELECT Filename FROM bestand WHERE joined='$SESSION['clientid']'";
					*/
	$stmt = mysqli_stmt_init($conn);
	//check of statement goed gaat
	if(!mysqli_stmt_prepare($stmt, $viewdossier))
        {
            echo 'SQL statement failed.';
        }
		...
		mysqli_stmt_execute($stmt);  
        mysqli_stmt_close($stmt);
		header("Location: ../dossiertoevoegen.php?nieuwdossier=succes"); //deze nog ff updaten!!
